...
<script lang="ts" setup>
  ...
  import { batchGetPersonsByNodeTypes } from '@/api/swm/staffScheduling'; // 导入批量获取API
      ...
      return treeNodes.map(({ children, ...props }) => {
        ...
        const childrenSelected =
          children && children.length > 0
            ? areAllChildrenSelected({ key: props.key, title: props.title, children }, targetKeys)
            : false;
  ...
  async function handleSubmit() {
    ...
    console.log(props.formState, selectedLeafNodes);

    if (isSubmitting.value) return;
    isSubmitting.value = true;
    props.formState.bids = props.formState.beaconList.map((ele) => {
      return ele.id;
    });
    if (props.formState.areaName?.length > 0) {
    } else {
      message.warning('区域名称不能为空');
      isSubmitting.value = false;
      return;
    }
    if (props.formState.bids?.length > 2) {
    } else {
    ...
    saveAreaWithBeaconIds(props.formState)
      .then(async (result) => {
        message.success(props.formState.isEdit ? '保区域成功' : '新增区域成功！');
        // 关闭模态框
        closeModal();
