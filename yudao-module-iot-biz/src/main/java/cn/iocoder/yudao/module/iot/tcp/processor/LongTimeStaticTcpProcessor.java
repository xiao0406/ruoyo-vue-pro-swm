...
/**
 * TCP handler for long-time-static alarms.
 */
@Slf4j
@Component
public class LongTimeStaticTcpProcessor {
    ...
    private static final String WARNING_TYPE = "6";
    private static final String WARNING_CONTENT = "Long-time static alarm";
    private static final String VOICE_CONTENT = "Please confirm your safety status";
    ...
    public boolean canProcess(TcpMessageData messageData) {
        return messageData != null
                ...
                && (messageData.getAlarmValue() & LONG_TIME_STATIC_ALARM_BIT_MASK) != 0;
    }
    ...
    public void process(TcpMessageData messageData, ChannelHandlerContext ctx) {
        ...
        if (StrUtil.isBlank(idCard)) {
            log.warn("Skip long-time-static warning because device is not bound to a person, deviceId={}", deviceId);
            return;
        }
        ...
        if (tdResult.getCode() == R.FAIL) {
            log.warn("Long-time-static TDengine save failed, deviceId={}, msg={}", deviceId, tdResult.getMsg());
        }
    ...
    public boolean hasLongTimeStaticAlarmFromRawMessage(String rawMessage) {
        ...
        return alarmValue != null && (alarmValue & LONG_TIME_STATIC_ALARM_BIT_MASK) != 0;
    }
    ...
    private Integer parseAlarmValue(String rawMessage) {
        ...
        try {
            ...
            log.warn("Cannot parse long-time-static alarm value from raw tcp message: {}", rawMessage, ex);
            return null;
        }
    }
