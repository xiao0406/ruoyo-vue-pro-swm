...
/**
 * 长时间静止报警TCP消息处理器
 * 处理报警值中包含长时间静止报警位（Bit.6=1）的设备数据，并下发语音提醒
 * 与SOS报警不同，只要报警值包含长时间静止位就会处理，不要求纯长时间静止报警
 *
 * @author Shawn
 * @date 2025-01-28
 */
@Component
public class LongTimeStaticTcpProcessor {

    private static final Logger logger = LoggerFactory.getLogger(LongTimeStaticTcpProcessor.class);
    ...
    // 长时间静止报警在第6位（从0开始），对应的值是2^6=64
    private static final int LONG_TIME_STATIC_ALARM_BIT_MASK = 64;

    // 字典配置常量
    private static final String DICT_TYPE = "klt_xiafa_yy_cs";
    private static final String DICT_KEY_LONG_TIME_STATIC = "long_time_static";
    private static final String DEFAULT_VOICE_TEXT = "长时间静止报警已触发";

    /**
     * 判断是否可以处理该消息
     * 只要报警值包含长时间静止报警位（Bit.6=1）就处理
     *
     * @param messageData TCP消息数据
     * @return true表示包含长时间静止报警
     */
    public boolean canProcess(TcpMessageData messageData) {
        ...
        // 判断是否包含长时间静止报警（Bit.6=1）
        boolean hasLongTimeStaticAlarm = (alarmValue & LONG_TIME_STATIC_ALARM_BIT_MASK) != 0;

        if (hasLongTimeStaticAlarm) {
            logger.info("检测到长时间静止报警, 设备ID: {}, 报警值: {} (二进制: {})",
                messageData.getDeviceId(),
                alarmValue,
                Integer.toBinaryString(alarmValue));
        }

        return hasLongTimeStaticAlarm;
    }
    ...
    public void process(TcpMessageData messageData, ChannelHandlerContext ctx) {
        ...
        try {
            logger.info("开始处理长时间静止报警, 设备ID: {}, 报警值: {}",
...
//            String areaName = tcpBeaconLocationService.getAreaNameFromBeacons(messageData);
            ...
            logger.info("长时间静止报警位置信息: deviceId={}, location={}, areaName={}",
                deviceId, location, areaName);

            // 3. 保存长时间静止报警数据到TDengine
            saveLongTimeStaticDataToTDengine(deviceId, messageData, btSignalInfo);

            // 4. 保存到swm_warning_management表（type=6表示长时间静止报警）
            saveWarningToSwmWarningManagement(deviceId, "6", btSignalInfo, location, areaName);

            // 5. 从字典获取语音提示内容
            String voiceText = DictUtils.getDictLabel(DICT_TYPE, DICT_KEY_LONG_TIME_STATIC, DEFAULT_VOICE_TEXT);
            logger.debug("获取语音提示内容 - 字典类型: {}, 字典键: {}, 获取值: {}",
                DICT_TYPE, DICT_KEY_LONG_TIME_STATIC, voiceText);
            ...
            if (!messageData.isZTDevice()){
                ...
                logger.info("长时间静止报警处理完成, 设备ID: {}, 语音内容: {}, 语音指令: {}",
            ...
            logger.error("处理长时间静止报警时发生异常, 设备ID: {}", deviceId, e);
        }
    }
    ...
    /**
     * 保存长时间静止报警数据到TDengine
     *
     * @param deviceId 设备ID
     * @param messageData TCP消息数据
     * @param btSignalInfo 蓝牙信标信息JSON
     */
    private void saveLongTimeStaticDataToTDengine(String deviceId, TcpMessageData messageData, String btSignalInfo) {
        try {
            // 构建长时间静止报警数据Map
            Map<String, Object> longTimeStaticDataMap = new HashMap<>();
            longTimeStaticDataMap.put("device_id", deviceId);
            longTimeStaticDataMap.put("type", "6"); // 长时间静止报警
            longTimeStaticDataMap.put("act", "ca_sos"); // 保持与WebSocket一致

            // 添加sos_time字段（使用扫描时间戳）
            if (messageData.getScanTimestamp() != null) {
                longTimeStaticDataMap.put("sos_time", messageData.getScanTimestamp());
            }

            // 添加蓝牙信标信息
            if (StringUtils.isNotBlank(btSignalInfo)) {
                longTimeStaticDataMap.put("bt_signal_info", btSignalInfo);
            }

            // 调用服务保存数据
            R<JSONObject> result = helmetSosTdEngineService.saveHelmetSosData(deviceId, longTimeStaticDataMap);

            if (result.getCode() != R.SUCCESS) {
                logger.error("保存长时间静止报警数据到TDengine失败, deviceId: {}, 错误: {}",
                    deviceId, result.getMsg());
            } else {
                logger.info("成功保存长时间静止报警数据到TDengine, deviceId: {}", deviceId);
            }

        } catch (Exception e) {
            logger.error("保存长时间静止报警数据到TDengine异常, deviceId: {}", deviceId, e);
        }
    }
...
//            String idCard = !results.isEmpty() ? results.get(0) : null;
            ...
            // 构建预警信息
//            String warningContent = "长时间静止报警";
            String triggerReason = "检测到工人长时间处于静止状态";
            String alarmRecord = "安全帽长时间静止报警记录";
            String warningType = "2"; // 被动报警
            String warningContent = DictUtils.getDictLabel("warning_content_enum", "长时间静止报警", "长时间静止报警");
    ...
    /**
     * 直接从原始报文判断是否包含长时间静止报警
     * 这个方法可以独立使用，不依赖TcpMessageData对象
     *
     * @param rawMessage 原始TCP报文
     * @return true表示包含长时间静止报警（报警值Bit.6=1）
     */
    public boolean hasLongTimeStaticAlarmFromRawMessage(String rawMessage) {
        try {
            // 验证基本格式
            ...
            // 判断是否包含长时间静止报警
            boolean hasLongTimeStatic = (alarmValue & LONG_TIME_STATIC_ALARM_BIT_MASK) != 0;

            if (hasLongTimeStatic) {
                logger.info("从原始报文检测到长时间静止报警, 报警值: 0x{} (二进制: {})",
                    alarmField, Integer.toBinaryString(alarmValue));
            }

            return hasLongTimeStatic;
