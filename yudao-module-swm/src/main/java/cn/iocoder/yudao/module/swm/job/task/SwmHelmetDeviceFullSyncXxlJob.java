...
public class SwmHelmetDeviceFullSyncXxlJob {
    ...
    public void swmHelmetDeviceFullSyncHandler() {
        ...
        try {
            // 使用 ZJGGGD 租户上下文查询设备（ZJGGGD tenantId = 1）
            List<SwmHelmetDeviceDO> deviceList = new ArrayList<>();
            TenantUtils.execute(1L, () -> {
                List<SwmHelmetDeviceDO> list = swmHelmetDeviceMapper.selectList(
                        new LambdaQueryWrapperX<SwmHelmetDeviceDO>()
                                .eq(SwmHelmetDeviceDO::getStatus, "0")
                );
                if (list != null) {
    ...
    private void sendFullSyncMq(List<SwmHelmetDeviceDO> deviceList, String jobParam) {
        int totalCount = deviceList.size();
        ...
        for (int i = 0; i < batchNum; i++) {
            ...
            List<SwmHelmetDeviceDO> batchDeviceList = deviceList.subList(start, end);

            try {
                mqSendUtil.sendDeviceBatchChangeMsg(SyncDataOperateTypeEnum.HELMET_FULL_SYNC.getCode(), batchDeviceList);
                XxlJobHelper.log("第{}批MQ发送成功，条数：{}", i + 1, batchDeviceList.size());
            } catch (Exception e) {
