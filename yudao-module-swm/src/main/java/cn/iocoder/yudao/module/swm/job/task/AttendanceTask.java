...
import cn.iocoder.yudao.module.swm.service.*;
...
public class AttendanceTask {
    ...
    /**
     * 需要按人员绑定工作区计算考勤的租户 ID（原 ZJGGJS=2, ZJZK=4）
     */
    private static final Set<Long> PERSONAL_WORK_AREA_TENANT_IDS = Set.of(2L, 4L);
    ...
    public void generateMonthlyDailyAttendance() {
        SwmJobLogDO jobLog = new SwmJobLogDO();
        // DFXCYY tenantId = 6
        Long tenantId = 6L;
        TenantUtils.execute(tenantId, () -> {
            ...
            // 1.先从排班表查询所有人员
            SwmPersonScheduleDO swmPersonSchedule = new SwmPersonScheduleDO();
            List<SwmPersonScheduleDO> schedules = swmPersonScheduleService.findList(swmPersonSchedule);
            ...
            while (!calendar.getTime().after(nowDate)) {
                ...
                generateOneDayAttendance(currentDay, schedules);
     ...
     * 生成某一天的考勤数据
     */
    private void generateOneDayAttendance(Date targetDate, List<SwmPersonScheduleDO> scheduleList) {
        ...
        // 2. 查询当天是否已有考勤（有则跳过）
        SwmDailyAttendanceDO attendanceParam = new SwmDailyAttendanceDO();
        attendanceParam.setAttendanceDate(targetDateToLocalDate(targetDate));
        List<SwmDailyAttendanceDO> existList = swmDailyAttendanceService.findList(attendanceParam);
        ...
        // 3. 批量生成考勤
        List<SwmDailyAttendanceDO> insertList = new ArrayList<>();

        for (SwmPersonScheduleDO schedule : scheduleList) {
            SwmDailyAttendanceDO att = new SwmDailyAttendanceDO();
            ...
            att.setAttendanceDate(targetDateToLocalDate(targetDate));
            ...
            try {
                ...
                if ("1".equals(classes)) {
                    // ==================== 早班 ====================
                    // 打卡开始时间 01:40:00
                    att.setClockStartTime(parseToLocalDateTime(sdfDateTime.parse(sdf.format(targetDate) + " 01:40:00")));
                    // 打卡结束时间 次日13:40:00
                    Date nextDay = DateUtil.offsetDay(targetDate, 1);
                    att.setClockEndTime(parseToLocalDateTime(sdfDateTime.parse(sdf.format(nextDay) + " 13:40:00")));
                    ...
                    // ==================== 夜班 ====================
                    att.setClockStartTime(parseToLocalDateTime(sdfDateTime.parse(sdf.format(targetDate) + " 13:40:00")));
                    Date nextDay = DateUtil.offsetDay(targetDate, 1);
                    att.setClockEndTime(parseToLocalDateTime(sdfDateTime.parse(sdf.format(nextDay) + " 13:40:00")));
                ...
                // 打卡时间（Date类型）
                att.setClockInTime(parseToLocalDateTime(clockInTimeDate));
                att.setClockOutTime(parseToLocalDateTime(clockOutTimeDate));
                att.setClockInDate(parseToLocalDateTime(clockInTimeDate));
                att.setClockOutDate(parseToLocalDateTime(clockOutTimeDate));
...
// ==================== 工时计算（按你最新规则） ====================
        ...
        if (CollectionUtils.isNotEmpty(insertList)) {
            List<List<SwmDailyAttendanceDO>> lists = BatchOperationsUtil.batchCutting(insertList, 50);
            for (List<SwmDailyAttendanceDO> insertData : lists) {
...
//            swmDailyAttendanceService.saveBatch(insertList);
    ...
    public void calculateIdleHours() {
        ...
        //获取系统所有租户信息
        List<TenantDO> corpList = tenantService.getTenantListByStatus(CommonStatusEnum.ENABLE.getStatus());
        ...
        for (TenantDO tenant : corpList) {
            ...
            try {
                //设置当前线程的租户信息
                // TenantUtils.execute handles context
                ...
                // 查询指定日期的所有考勤记录
                SwmDailyAttendanceDO query = new SwmDailyAttendanceDO();
                query.setAttendanceDate(targetDateToLocalDate(targetDate));
                List<SwmDailyAttendanceDO> attendanceList = swmDailyAttendanceService.findList(query);
                ...
                // 为每条考勤记录计算怠工时长
                for (SwmDailyAttendanceDO record : attendanceList) {
                    try {
                        // 获取员工身份证号
                        ...
                        if (idCard != null) {
                            ...
                            // @author: Shawn
                            // @date: 2025/01/27
                            LocalDateTime clockInTimeLdt = record.getClockInTime();
                            LocalDateTime clockOutTimeLdt = record.getClockOutTime();
                            Date clockInTime = localDateTimeToDate(clockInTimeLdt);
                            Date clockOutTime = localDateTimeToDate(clockOutTimeLdt);
                            ...
                            if (clockInTime == null || clockOutTime == null) {
                                try {
                                    ...
                                    String recordDateStr = dateFormat.format(
                                            Date.from(record.getAttendanceDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
                                    CommonResult<Map<String, Object>> tdengineResult = helmetTdengineService
                                            .getFirstAndLastTimeByIdCardAndDate(idCard, recordDateStr, workTimeRange);

                                    if (tdengineResult.getCode() == 0) {
                ...
                jobLog.setDuration(java.time.Duration.between(jobLog.getStartTime(), jobLog.getEndTime()).toMillis());
                swmJobLogService.save(jobLog);
            }
        }
    }
    ...
    public void calculateMonthlyAttendance() {
        ...
        for (TenantDO tenant : corpList) {
            ...
            try {
                ...
                // 2. 查询当月所有员工的日考勤记录
                SwmDailyAttendanceDO query = new SwmDailyAttendanceDO();
                query.setBeginAttendanceDate(targetDateToLocalDate(startDate));
                query.setEndAttendanceDate(targetDateToLocalDate(endDate));
                // query.setEmployeeId("1935182658540556288"); // 注意，测试使用生产上要去掉，先写死。
                List<SwmDailyAttendanceDO> dailyAttendanceList = swmDailyAttendanceService.findList(query);

                // 3. 按员工ID分组
                Map<String, List<SwmDailyAttendanceDO>> attendanceByEmployee = dailyAttendanceList.stream()
                        .collect(Collectors.groupingBy(SwmDailyAttendanceDO::getEmployeeId));
                ...
                // 5. 处理每个员工的考勤数据
                for (Map.Entry<String, List<SwmDailyAttendanceDO>> entry : attendanceByEmployee.entrySet()) {
                    String employeeId = entry.getKey();
                    List<SwmDailyAttendanceDO> employeeAttendance = entry.getValue();

                    // 获取员工基本信息
                    SwmPersonDO person = swmPersonService.getPerson(employeeId);
                    if (person == null) {
                    ...
                    SwmAttendanceSummaryDO existingSummary = swmAttendanceSummaryService.findByEmployeeIdAndMonth(employeeId,
                            monthStr);

                    // 7. 保存或更新记录
                    SwmAttendanceSummaryDO summary = existingSummary != null ? existingSummary : new SwmAttendanceSummaryDO();
                    ...
                    SwmPersonScheduleDO queryPersonSchedule = new SwmPersonScheduleDO();
                    queryPersonSchedule.setIdCard(person.getIdentityCard());
                    queryPersonSchedule.setMonth(monthStr);
                    List<SwmPersonScheduleDO> personScheduleList = swmPersonScheduleService.findList(queryPersonSchedule);
    ...
    public void calculateMonthlyAttendanceV2() {
        ...
        try {
            ...
            Map<String, List<SwmDailyAttendanceDO>> monthlyDataCache = 
            ...
            jobLog.setEndTime(LocalDateTime.now());
            jobLog.setDuration(jobLog.getEndTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli() - jobLog.getStartTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli());
            swmJobLogService.save(jobLog);
        }
    }
    ...
    public void createDailyAttendance() {
        ...
        try {
            ...
            query.setPersonnelStatus("1"); // 在职状态
            ...
            List<SwmPersonDO> activePersons = swmPersonService.findList(query);
            ...
            // 3. 为每个在职人员创建/更新考勤记录
            for (SwmPersonDO person : activePersons) {
                // 3.1 检查是否已有当天的考勤记录（使用专门的查询方法确保排重）
                SwmDailyAttendanceDO existingAttendance = swmDailyAttendanceService
                        .findByEmployeeIdAndDate(person.getId(), targetDateToLocalDate(today));
                ...
                SwmScheduleTimeDO scheduleTime = scheduleInfo.scheduleTime;
                ...
                if (existingAttendance == null) {
                    // 创建新记录
                    SwmDailyAttendanceDO newAttendance = new SwmDailyAttendanceDO();
                    ...
                    newAttendance.setAttendanceDate(targetDateToLocalDate(today));
     ...
     */
    private ScheduleInfo getScheduleTimeForPerson(SwmPersonDO person, Date date) {
        // ========== 1. 入参判空 + 基础日志 ==========
        if (person == null) {
        ...
        try {
            ...
            // ========== 3. 查询员工排班信息 + 日志 ==========
            List<SwmPersonScheduleDO> personScheduleList = null;
            try {
            ...
            // ========== 4. 获取第一个排班记录 + 日志 ==========
            SwmPersonScheduleDO personSchedule = personScheduleList.get(0);
            ...
            // ========== 5. 查询班次对应的时间 + 日志 ==========
            SwmScheduleTimeDO scheduleTimeQuery = new SwmScheduleTimeDO();
            scheduleTimeQuery.setShiftType(classes);
            List<SwmScheduleTimeDO> scheduleTimeListOld = null;
            List<SwmScheduleTimeDO> scheduleTimeList = new ArrayList<>();
            try {
//                scheduleTimeList = swmScheduleTimeService.findList(scheduleTimeQuery);
                ...
                XxlJobHelper.log("【ERROR】getScheduleTimeForPerson：查询班次时间失败，人员：{}，班次：{}，异常：{}，人员租户：{}",
                        person.getName(),classes, e.getMessage(), "");
                throw e; // 抛出异常，让上层捕获
            }
            ...
            // ========== 6. 获取第一个班次时间 + 日志 ==========
            SwmScheduleTimeDO scheduleTime = scheduleTimeList.get(0);
     ...
     * @return 应考勤时长(小时)
     */
    private BigDecimal calculateScheduledHours(SwmScheduleTimeDO scheduleTime) {
     ...
     * @date: 2025/01/27
     */
    private BigDecimal applyActualHoursFallback(SwmDailyAttendanceDO record, BigDecimal calculatedActualHours,
            String context) {
     ...
     * @date 2025/01/27
     */
    private void updateAttendanceStatusByNewRule(SwmDailyAttendanceDO record) {
...
//        }
     ...
     * @date 2025/01/09
     */
    private boolean isNormalAttendanceCondition(SwmDailyAttendanceDO record) {
        LocalDateTime clockInDate = record.getClockInDate();
        LocalDateTime clockOutDate = record.getClockOutDate();
     ...
     * @date 2025/01/27
     */
    private boolean isAbnormalAttendanceCondition(SwmDailyAttendanceDO record) {
        LocalDateTime clockInDate = record.getClockInDate();
        LocalDateTime clockOutDate = record.getClockOutDate();
        BigDecimal effectiveWorkHours = record.getEffectiveWorkHours();
        BigDecimal actualHours = record.getActualHours();
     ...
     * @date 2025/01/27
     */
    private boolean isNotAttendanceCondition(SwmDailyAttendanceDO record) {
        LocalDateTime clockInDate = record.getClockInDate();
        LocalDateTime clockOutDate = record.getClockOutDate();
        BigDecimal effectiveWorkHours = record.getEffectiveWorkHours();
        BigDecimal actualHours = record.getActualHours();
    ...
    public void calculateAttendanceByTimeRange() {
        ...
        Date date = new Date();
        jobLog.setStartTime(parseToLocalDateTime(date));
        jobLog.setExecuteStatus("1"); // 默认失败
        ...
        for (TenantDO tenant : corpList) {
            ...
            try {
                ...
                // 1. 解析参数
                SwmDailyAttendanceDO params = new SwmDailyAttendanceDO();
                params.setAttendanceDate(targetDateToLocalDate(date));
                // 2. 查询待处理记录
                List<SwmDailyAttendanceDO> records = swmDailyAttendanceService.findList(params);
    ...
    public void processclockInCard(String requestDate) {
        ...
        List<SwmDailyAttendanceDO> records = swmDailyAttendanceService.findClockInCardList(date,random);
//        List<SwmDailyAttendanceDO> records = queryPendingAttendanceRecords(params);
        ...
        List<SwmDailyAttendanceDO> onlineDevices = Collections.synchronizedList(new ArrayList<>());
        List<CompletableFuture<Void>> futures = new ArrayList<>();
        for (SwmDailyAttendanceDO item : records) {
            if (item.getClockInDate() == null) {
                CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                    try {
                        String deviceId = item.getDeviceId();
                        Date clockStartTime = localDateTimeToDate(item.getClockStartTime());
                        ...
                        if (clockStartTime != null){
                            ...
                            if (StrUtil.isNotEmpty(clockInTimeStr)) {
                                // 统一去掉毫秒（如果有）
                                ...
                                // Convert to Date for compatibility
                                item.setClockInDate(clockInDateTime);
                                item.setClockInTime(clockInDateTime);
        ...
        //批量更新
        List<List<SwmDailyAttendanceDO>> lists = BatchOperationsUtil.batchCutting(onlineDevices, 50);
        for (List<SwmDailyAttendanceDO> list : lists) {
    ...
    public void processclockOutCard(String requestDate) {
        ...
        // 1. 查询两天所有的打卡记录
        Integer random = new Random().nextInt(1_000_000);
        List<SwmDailyAttendanceDO> records = swmDailyAttendanceService.findClockOutCardList(yestDay, nowDate, random);
        //List<SwmDailyAttendanceDO> records = queryPendingAttendanceRecords(params);
        ...
        List<SwmDailyAttendanceDO> clockOutRecords = new ArrayList<>();
        ...
        Map<String, SwmScheduleTimeDO> scheduleMap = new HashMap<>();
        List<SwmScheduleTimeDO> scheduleList = swmScheduleTimeService.findList(new SwmScheduleTimeDO());
        if (CollectionUtils.isNotEmpty(scheduleList)) {
            for (SwmScheduleTimeDO scheduleTime : scheduleList) {
                if (scheduleTime != null && StrUtil.isNotBlank(scheduleTime.getShiftType())) {
        ...
        for (SwmDailyAttendanceDO item : records) {
            try {
                ...
                //2.判断今天有没有数据，没有则跳过
                Integer dayCount = getLast3MinutesBluetoothCountByClockOut(deviceId, item.getIdentityCard(), startDay, nowDay);
                if (dayCount == null || dayCount == 0) {
                    continue;
                }

                // 3. 查询最近3分钟蓝牙信号
                Integer count = getLast3MinutesBluetoothCountByClockOut(deviceId, item.getIdentityCard(), startTime, endTime);

                boolean isDayShift = "1".equals(item.getClasses());
                SwmScheduleTimeDO scheduleTime = scheduleMap.get(item.getClasses());
                Date nowTime = new Date();
                LocalDateTime nowTimeLdt = parseToLocalDateTime(nowTime);
                ...
                if (count != null && count > 0) {
                    ...
                    if (isDayShift
                        ...
                        item.setAfterStartDate(nowTimeLdt);
                        item.setAfterStartTime(nowTimeLdt);
                ...
                if (count != null && count == 0) {
                    ...
                    if (isDayShift && item.getNoonEndDate() == null && isInWindow(nowTime, noonStartWindow, noonEndWindow)) {
                        ...
                        Integer noonWindowSignalCount = getLast3MinutesBluetoothCountByClockOut(
                                deviceId, item.getIdentityCard(), noonStartStr, noonRangeEndStr);
                        if (noonWindowSignalCount != null && noonWindowSignalCount > 0) {
                            item.setNoonEndDate(nowTimeLdt);
                            item.setNoonEndTime(nowTimeLdt);
                            clockOutRecords.add(item);
...
//                        continue;
                    ...
                    if (item.getClockOutDate() == null && canCompensateFinalClockOut) {
                        item.setClockOutDate(nowTimeLdt);
                        item.setClockOutTime(nowTimeLdt);
        ...
        //批量更新
        List<List<SwmDailyAttendanceDO>> lists = BatchOperationsUtil.batchCutting(clockOutRecords, 50);
        for (List<SwmDailyAttendanceDO> list : lists) {
            swmDailyAttendanceService.updateBatch(list);
        }
    }
    ...
    private Date buildWindowTime(LocalDate attendanceDate, String hhmmTime, int offsetMinutes) {
        if (attendanceDate == null || StrUtil.isBlank(hhmmTime)) {
            return null;
        }
        try {
            String normalized = hhmmTime.length() >= 5 ? hhmmTime.substring(0, 5) : hhmmTime;
            String dateTimeStr = attendanceDate.toString() + " " + normalized;
     ...
     */
    private Integer getLast3MinutesBluetoothCountByClockOut(String deviceId,String idCard, String start, String end) {
        Long tenantId = TenantContextHolder.getTenantId();
        ...
        try {
            ...
            CommonResult<JSONObject> result = tdengineService.executeTDengineSQLByXXJOB(sql, tenantId);
     ...
     * @date 2025-08-12
     */
    private List<SwmDailyAttendanceDO> queryPendingAttendanceRecords(AttendanceParams params) {
        // 使用 MyBatis-Plus 查询待处理的考勤记录
        SwmDailyAttendanceDO query = new SwmDailyAttendanceDO();
        query.setStatus("0"); // 只查询未处理的记录
        // 将 Date 转换为 LocalDateTime 用于查询
        query.setBeginAttendanceDate(targetDateToLocalDate(params.startTime));
        query.setEndAttendanceDate(targetDateToLocalDate(params.endTime));
        // 如果指定了身份证，设置查询条件
        if (!params.idCardList.isEmpty()) {
            // 使用 idCards 字段传递身份证列表（@TableField(exist = false)）
            query.setIdCards(params.idCardList);
        }

        List<SwmDailyAttendanceDO> records = swmDailyAttendanceService.findList(query);
     ...
     * @date 2025-08-12
     */
    private AttendanceResult processAttendanceRecords(List<SwmDailyAttendanceDO> records) {
        AttendanceResult result = new AttendanceResult();
        result.totalCount = records.size();
        Long tenantId = TenantContextHolder.getTenantId();
        
        for (SwmDailyAttendanceDO record : records) {
            try {
     ...
     * @date 2025-08-12
     */
    private boolean processAttendanceRecord(SwmDailyAttendanceDO record) {
        ...
        if (shouldCalculateActualHours(record)  || shouldCalculateActualHoursOther(record)) {
            ...
            // 特定租户：根据人员绑定的工作区计算
            if (isPersonalWorkAreaCorp(tenantId)) {
                workAreaHours = calculateWorkAreaHoursForZJGGJS(record);
            } else {
                workAreaHours = calculateWorkAreaHours(record);
            }
        ...
        if (shouldCalculateWorkHours(record)) {
            ...
            // 特定租户：根据人员绑定的工作区计算
            if (isPersonalWorkAreaCorp(tenantId)) {
                workHours = calculateWorkAreaHours24hForZJGGJS(record);
            } else {
                workHours = calculateWorkAreaHours24h(record);
            }
            if (workHours != null) {
        ...
        if (shouldCalculateWorkHours(record)) {
            ...
            if (isPersonalWorkAreaCorp(tenantId)) {
                ...
                idleHours = calculateIdleAreaHours(record);
            } else {
                idleHours = calculateIdleAreaHours(record);
            }
            if (idleHours != null) {
        ...
        if (updated) {
            ...
            if (record.getScheduledHours() == null || record.getScheduledHours().compareTo(BigDecimal.ZERO) <= 0) {
                if (StrUtil.isNotBlank(record.getWorkTimeRange())) {
     ...
     * @date 2025-08-12
     */
    private boolean shouldCalculateActualHours(SwmDailyAttendanceDO record) {
     ...
     */
    private boolean shouldCalculateActualHoursOther(SwmDailyAttendanceDO record) {
        // 1. 打卡时间不全（核心判断）
     ...
     * @date 2025-08-12
     */
    private void calculateAndUpdateActualHours(SwmDailyAttendanceDO record) {
        BigDecimal actualHours = calculateScheduledHoursByClockTime(
            localDateTimeToDate(record.getClockInTime()), localDateTimeToDate(record.getClockOutTime()));
        record.setActualHours(actualHours);
     ...
     * @date 2025-08-12
     */
    private void markAsProcessed(SwmDailyAttendanceDO record) {
        record.setStatus("1");
    ...
    private static class ScheduleInfo {
        SwmScheduleTimeDO scheduleTime;
        String classes; // 班次名称
        
        ScheduleInfo(SwmScheduleTimeDO scheduleTime, String classes) {
    ...
    public void createDailyAttendanceV2() {
        SwmJobLogDO jobLog = initJobLog("createDailyAttendanceV2");
        ...
        for (TenantDO tenant : corpList) {
            ...
            try {
                ...
                // 2. 查询目标人员
                List<SwmPersonDO> targetPersons = queryTargetPersons(params);
    ...
    private AttendanceGenerationParams parseAttendanceGenerationParams() {
        ...
        if (StrUtil.isNotBlank(jobParam)) {
            ...
            if (StrUtil.isNotBlank(params.idCard)) {
            ...
            if (StrUtil.isNotBlank(idCards)) {
                ...
                for (String id : idCardArray) {
                    ...
                    if (StrUtil.isNotBlank(trimmedId) && !params.idCardList.contains(trimmedId)) {
     ...
     * 查询目标人员
     */
    private List<SwmPersonDO> queryTargetPersons(AttendanceGenerationParams params) {
        Long tenantId = TenantContextHolder.getTenantId();
        // 如果有身份证列表，查询指定的人员
        if (!params.idCardList.isEmpty()) {
        ...
        // 向后兼容：如果只有单个身份证参数
        else if (StrUtil.isNotBlank(params.idCard)) {
     ...
     * 查询单个人员
     */
    private List<SwmPersonDO> querySinglePerson(String idCard) {
        List<SwmPersonDO> targetPersons = new ArrayList<>();
        SwmPersonDO person = swmPersonService.getByIdentityCard(idCard);
        ...
        if (person != null 
            ...
            } else if (!"1".equals(person.getPersonnelStatus())) {
     ...
     * @return 符合条件的在职人员列表
     */
    private List<SwmPersonDO> queryMultiplePersons(List<String> idCardList) {
        List<SwmPersonDO> targetPersons = new ArrayList<>();
        int successCount = 0;
        int skipCount = 0;
     ...
     * 查询所有在职人员
     */
    private List<SwmPersonDO> queryAllActivePersons() {
        SwmPersonDO query = new SwmPersonDO();
        query.setPersonnelStatus("1");
        query.setStatus("0");
     ...
     * 生成考勤记录
     */
    private AttendanceGenerationResult generateAttendanceRecords(List<SwmPersonDO> persons, Date targetDate) {
        ...
        for (SwmPersonDO person : persons) {
     ...
     * 处理单个人员
     */
    private ProcessResult processSinglePerson(SwmPersonDO person, Date targetDate, String dateStr) {
        try {
            // 验证身份证
            ...
            // 检查是否已存在
            SwmDailyAttendanceDO existing = swmDailyAttendanceService.findByIdentityCardAndDate(person.getIdentityCard(), targetDateToLocalDate(targetDate));
     ...
     * 处理已存在的考勤记录
     */
    private ProcessResult handleExistingAttendance(SwmDailyAttendanceDO existing, SwmPersonDO person, 
        ...
        // 尝试更新排班信息（如果原记录没有）
        if (StrUtil.isEmpty(existing.getWorkTimeRange()) || StrUtil.isEmpty(existing.getClasses())) {
     ...
     * 创建新的考勤记录
     */
    private void createNewAttendance(SwmPersonDO person, Date targetDate) {
        SwmDailyAttendanceDO attendance = buildNewAttendance(person, targetDate);
     ...
     */
    private SwmDailyAttendanceDO buildNewAttendance(SwmPersonDO person, Date targetDate) {
        // 日期格式化工具（避免重复创建）
        ...
        try {
            SwmDailyAttendanceDO attendance = new SwmDailyAttendanceDO();
            ...
            // 考勤日期（已兜底，必赋值）
            attendance.setAttendanceDate(targetDateToLocalDate(targetDate));
    ...
    private BigDecimal calculateScheduledHoursFromWorkTimeRange(String workTimeRange, Double restTime) {
        if (StrUtil.isBlank(workTimeRange)) {
            return BigDecimal.ZERO;
        }
     ...
     * @date 2025-08-14
     */
    private boolean isRestDay(SwmScheduleTimeDO scheduleTime, Date targetDate) {
        if (scheduleTime == null || StrUtil.isBlank(scheduleTime.getRestDays())) {
            return false;
        }
    ...
    private void setScheduleInfo(SwmDailyAttendanceDO attendance, SwmPersonDO person, Date targetDate) {
        // ========== 1. 入参判空日志 ==========
        if (attendance == null) {
...
//            attendance.setRestTime(BigDecimal.ZERO);
        ...
        // 拼接时间范围，空值兜底
        String workTimeRange = (StrUtil.isEmpty(startTime) ? "未知开始时间" : startTime)
                + "-" + (StrUtil.isEmpty(endTime) ? "未知结束时间" : endTime);

        // ========== 5. 班次信息判空日志 ==========
        String classes = scheduleInfo.classes;
        if (StrUtil.isEmpty(classes)) {
     ...
     * 设置默认值
     */
    private void setDefaultValues(SwmDailyAttendanceDO attendance) {
     ...
     * 验证身份证
     */
    private boolean validateIdentityCard(SwmPersonDO person) {
        if (StrUtil.isEmpty(person.getIdentityCard())) {
     ...
     * 初始化任务日志
     */
    private SwmJobLogDO initJobLog(String jobName) {
        SwmJobLogDO jobLog = new SwmJobLogDO();
     ...
     * 保存任务日志
     */
    private void saveJobLog(SwmJobLogDO jobLog) {
        jobLog.setEndTime(LocalDateTime.now());
        jobLog.setDuration(jobLog.getEndTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli() - jobLog.getStartTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli());
        swmJobLogService.save(jobLog);
    }
     ...
     * @date 2025-08-13
     */
    private boolean shouldCalculateWorkHours(SwmDailyAttendanceDO record) {
        // 必须有身份证号
        if (StrUtil.isBlank(record.getIdentityCard())) {
            return false;
        }
        ...
        return record.getClockInDate() != null || 
               ...
               StrUtil.isNotBlank(record.getWorkTimeRange());
    }
     ...
     * @date 2025-08-13
     */
    private BigDecimal calculateAreaHours(SwmDailyAttendanceDO record, String areaType, String areaName) {
        Long tenantId = TenantContextHolder.getTenantId();
        try {
            // 1. 确定查询时间范围
     ...
     * @date 2025-08-13
     */
    private BigDecimal calculateWorkAreaHours(SwmDailyAttendanceDO record) {
        Long tenantId = TenantContextHolder.getTenantId();
     ...
     * @date 2025-08-13
     */
    private BigDecimal calculateIdleAreaHours(SwmDailyAttendanceDO record) {
        Long tenantId = TenantContextHolder.getTenantId();
    ...
    /**
     * 判断当前租户是否需要按人员绑定工作区计算考勤
     * @param tenantId 租户ID
     * @return true-需要按人员绑定工作区计算，false-使用默认逻辑
     */
    private boolean isPersonalWorkAreaCorp(Long tenantId) {
        return tenantId != null && PERSONAL_WORK_AREA_TENANT_IDS.contains(tenantId);
    }

    /**
     * 根据 area_id 集合批量查询 SwmAreaDO 表获取区域类型映射
    ...
    private Map<String, String> buildAreaTypeMapFromIds(Set<String> areaIds) {
        ...
        try {
            // 使用 MyBatis-Plus 批量查询 SwmAreaDO 表
            List<SwmAreaDO> areaList = swmAreaService.findListByIds(areaIds);

            if (areaList != null && !areaList.isEmpty()) {
                for (SwmAreaDO area : areaList) {
                    if (StrUtil.isNotBlank(area.getId()) && StrUtil.isNotBlank(area.getAreaType())) {
     ...
     * @date 2025-08-20
     */
    private BigDecimal calculateWorkAreaHours24h(SwmDailyAttendanceDO record) {
        Long tenantId = TenantContextHolder.getTenantId();
        try {
            // 直接使用当天24小时范围
...
//            String endTime = dateStr + " 23:59:59";
     ...
     * @date 2026-06-16
     */
    private BigDecimal calculateWorkAreaHoursForZJGGJS(SwmDailyAttendanceDO record) {
        Long tenantId = TenantContextHolder.getTenantId();
        // 获取人员绑定的工作区
        ...
        if (boundAreaIds == null || boundAreaIds.isEmpty()) {
            XxlJobHelper.log("员工[{}]{}未绑定工作区，使用所有工作区计算",
                record.getEmployeeId(), record.getEmployeeName());
            return calculateWorkAreaHours(record);
        }
        ...
        List<WorkSegment> workSegments = queryAreaSegmentsForZJGGJS(
            record.getIdentityCard(), startTime, endTime, "0", boundAreaIds);
     ...
     * @date 2026-06-16
     */
    private BigDecimal calculateWorkAreaHours24hForZJGGJS(SwmDailyAttendanceDO record) {
        Long tenantId = TenantContextHolder.getTenantId();
        // 获取人员绑定的工作区
        List<String> boundAreaIds = getPersonBoundAreaIds(record.getIdentityCard());
        ...
        if (boundAreaIds == null || boundAreaIds.isEmpty()) {
            XxlJobHelper.log("员工[{}]{}未绑定工作区，使用所有工作区计算24小时时长",
                record.getEmployeeId(), record.getEmployeeName());
            return calculateWorkAreaHours24h(record);
        }
    ...
    private List<String> getPersonBoundAreaIds(String identityCard) {
        try {
            List<SwmPersonWorkAreaDO> workAreaList = swmPersonWorkAreaService.findActiveByIdentityCard(identityCard);
            ...
            return workAreaList.stream()
                .map(SwmPersonWorkAreaDO::getAreaId)
    ...
    private List<WorkSegment> queryAreaSegmentsForZJGGJS(String idCard, String startTime, String endTime,
                                                         String areaType, List<String> areaIds) {
        Long tenantId = TenantContextHolder.getTenantId();
        ...
        try {
            ...
            String sql = String.format(
                ...
                "AND time <= '%s' " +
                "ORDER BY time ASC",
                dbname, TdengineSuperTableConstants.AREA_FENCE_DATA,
                idCard, areaIdCondition, startTime, endTime
            );

            XxlJobHelper.log("ZJGGJS 查询指定工作区数据SQL: {}", sql);

            CommonResult<JSONObject> response = tdengineService.executeTDengineSQLByXXJOB(sql, tenantId);
     ...
     * @date 2025-12-29
     */
    private String[] determineQueryTimeRange(SwmDailyAttendanceDO record) {
        String attendanceDate = record.getAttendanceDate().toString();

        // 检查是否有班次信息
        if (StrUtil.isBlank(record.getClasses())) {
            return null;
        }
        ...
        try {
            if ("1".equals(classes)) {
                ...
                String nextDay = record.getAttendanceDate().plusDays(1).toString();
    ...
    private List<WorkSegment> queryAreaSegments(String idCard, String startTime, String endTime, String areaType) {
        ...
        try {
            ...
            String sql = String.format(
                ...
                "AND time <= '%s' " +
                "ORDER BY time ASC",
                dbname, TdengineSuperTableConstants.AREA_FENCE_DATA, idCard, startTime, endTime
            );
            ...
            // 根据 area_id 从 SwmAreaDO 表查询区域类型，建立映射
     ...
     * @date 2025-08-20
     */
    private void calculateAndSetClockTimeRange(SwmDailyAttendanceDO attendance, Date targetDate, String workTimeRange) {
        if (StrUtil.isBlank(workTimeRange) || !workTimeRange.contains("-")) {
        ...
        try {
            ...
            // 设置到考勤记录中
            attendance.setClockStartTime(parseToLocalDateTime(clockStartTime));
            attendance.setClockEndTime(parseToLocalDateTime(clockEndTime));
    ...
    private void logParameterExamples() {
        XxlJobHelper.log("=== 参数使用示例 ===");
        XxlJobHelper.log("month=2025-01           // 指定月份");
        XxlJobHelper.log("idCard=330xxx           // 单个身份证");
        XxlJobHelper.log("idCards=330xxx,331xxx   // 多个身份证（逗号分隔）");
        XxlJobHelper.log("month=2025-01;idCard=330xxx  // 组合参数（分号分隔）");
    }
    ...
    private List<String> getTargetIdCardsFromCache(MonthlyAttendanceParams params, 
                                                  Map<String, List<SwmDailyAttendanceDO>> dataCache) {
        if (!params.idCardList.isEmpty()) {
    ...
    private boolean updateActualDaysToSummary(String identityCard, String targetMonth, BigDecimal actualDays) {
        try {
            // 查询现有记录
            List<SwmAttendanceSummaryDO> existingRecords = swmAttendanceSummaryService
                .findByIdentityCardAndMonth(identityCard, targetMonth);
            
            if (!existingRecords.isEmpty()) {
                // 更新现有记录的实际出勤天数
                for (SwmAttendanceSummaryDO record : existingRecords) {
    ...
    private List<String> getTargetIdCards(MonthlyAttendanceParams params) {
        ...
        try {
            ...
            List<SwmPersonScheduleDO> scheduleList = swmPersonScheduleService.findList(query);
            
            List<String> idCardList = new ArrayList<>();
            Set<String> idCardSet = new HashSet<>(); // 用于去重
            
            for (SwmPersonScheduleDO schedule : scheduleList) {
                if (StrUtil.isNotBlank(schedule.getIdCard()) && !idCardSet.contains(schedule.getIdCard())) {
    ...
    private boolean createNewAttendanceSummaryRecord(String identityCard, String targetMonth, BigDecimal scheduledDays) {
        try {
            // 1. 查询人员基本信息
            SwmPersonDO person = swmPersonService.getByIdentityCard(identityCard);
            if (person == null) {
            ...
            // 2. 查询排班信息获取班次
            List<SwmPersonScheduleDO> scheduleList = swmPersonScheduleService
            ...
            // 3. 创建新的考勤统计记录
            SwmAttendanceSummaryDO summary = new SwmAttendanceSummaryDO();
     ...
     * @date 2025-08-21
     */
    private Map<String, List<SwmDailyAttendanceDO>> loadMonthlyAttendanceDataCache(String targetMonth) {
        try {
            XxlJobHelper.log("开始预加载{}月的日考勤数据到内存缓存", targetMonth);
            
            // 一次性查询整个月的所有日考勤记录
            List<SwmDailyAttendanceDO> allRecords = swmDailyAttendanceService.findByMonth(targetMonth);
            ...
            Map<String, List<SwmDailyAttendanceDO>> dataCache = allRecords.stream()
                .filter(record -> StrUtil.isNotBlank(record.getIdentityCard()))
                .collect(Collectors.groupingBy(SwmDailyAttendanceDO::getIdentityCard));
    ...
    private BigDecimal calculateActualDaysFromCache(String identityCard, 
        ...
        try {
            ...
            // 从缓存中获取该身份证号的所有日考勤记录
            List<SwmDailyAttendanceDO> records = dataCache.get(identityCard);
    ...
    private BigDecimal calculateScheduledDays(String identityCard, String targetMonth) {
        try {
            ...
            // 1. 查询员工排班信息
            List<SwmPersonScheduleDO> personScheduleList = swmPersonScheduleService
                .findByIdCardAndMonth(identityCard, targetMonth);
            ...
            SwmScheduleTimeDO scheduleTimeQuery = new SwmScheduleTimeDO();
            scheduleTimeQuery.setShiftType(classes);
            List<SwmScheduleTimeDO> scheduleTimeList = swmScheduleTimeService.findList(scheduleTimeQuery);
     ...
     * Date转LocalDate
     */
    private LocalDate targetDateToLocalDate(Date date) {
