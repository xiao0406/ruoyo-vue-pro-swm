...
export const dictTypeList = (params?: DictType | any) =>
  request.get<DictType>({ url: adminPath + '/sys/dictType/list', params });

export const dictTypeListData = (params?: DictType | any) =>
  request.post<Page<DictType>>({ url: adminPath + '/sys/dictType/listData', params });

export const dictTypeForm = (params?: DictType | any) =>
  request.get<DictType>({ url: adminPath + '/sys/dictType/form', params });

export const dictTypeSave = (params?: any, data?: DictType | any) =>
  request.postJson<DictType>({ url: adminPath + '/sys/dictType/save', params, data });

export const checkDictType = (oldDictType: string, dictType: string) =>
  request.get<DictType>({
    url: adminPath + '/sys/dictType/checkDictType',
    params: { oldDictType, dictType },
  });

export const dictTypeDelete = (params?: DictType | any) =>
  request.get<DictType>({ url: adminPath + '/sys/dictType/delete', params });

export const dictTypeTreeData = (params?: any) =>
  request.get<TreeDataModel[]>({ url: adminPath + '/sys/dictType/treeData', params });

export const refreshDictCache = () =>
  request.post<any>({ url: adminPath + '/sys/cache/refreshDictCache' });
