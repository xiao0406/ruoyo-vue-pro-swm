...
public class SwmHandleRecordController extends BaseController {
    ...
    @ResponseBody
    @ApiOperation("查询列表数据")
    public Page<SwmHandleRecord> listData(SwmHandleRecord swmHandleRecord, HttpServletRequest request, HttpServletResponse response) {
        // 创建分页对象
    ...
    public String save(@Validated SwmHandleRecord swmHandleRecord, HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(swmHandleRecord.getRecordName()) && StringUtils.isBlank(swmHandleRecord.getWarningRecord())) {
