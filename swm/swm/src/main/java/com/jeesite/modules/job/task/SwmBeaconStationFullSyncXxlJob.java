...
public class SwmBeaconStationFullSyncXxlJob {
    ...
    @Autowired
    private SwmBeaconStationDao swmBeaconStationDao;
    ...
    public void swmBeaconStationFullSyncHandler() {
        ...
        try {
            ...
            swmBeaconStation.setStatus("0"); // 仅同步正常状态的信标
            swmBeaconStation.getSqlMap().getWhere().disableAutoAddCorpCodeWhere(); // 禁用自动添加租户过滤
            // 支持按租户参数过滤
//            swmBeaconStation.setCorpCode(jobParam != null && !jobParam.trim().isEmpty() ? jobParam.trim() : null);
            List<SwmBeaconStation> beaconList = swmBeaconStationDao.findList(swmBeaconStation);
     ...
     */
    private void sendFullSyncMq(List<SwmBeaconStation> beaconList, String jobParam) {
        ...
        for (int i = 0; i < batchNum; i++) {
            ...
            List<SwmBeaconStation> batchBeaconList = beaconList.subList(start, end);
